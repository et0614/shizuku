  ...
  public class DateTimeCommunicator : PresentValueReadWriter
  {
    ...
    public bool SubscribeDateTimeCOV()
    {
      if (!covSubscribed)
      {
        ...
        covSubscribed = client.SubscribeCOVRequest(bacAddress, boID, (uint)MemberNumber.AccelerationRate, false, false, 3600);
      }
    ...
    /// <summary>計算遅延の有無を取得する</summary>
    /// <param name="succeeded">通信が成功したか否か</param>
    /// <returns>計算遅延の有無</returns>
    public bool GetIsEmulatorDelayed(out bool succeeded)
    {
    ...
    public bool GetIsEmulatorFinished(out bool succeeded)
    {
      return 1 == ReadPresentValue<uint>
