      ...
      lock (communicator.BACnetDevice)
      {
        ...
        for (int i = 0; i < vrfSystems.Length; i++)
        {
          ...
          for (int j = 0; j < vrf.VRFSystem.IndoorUnitNumber; j++)
          {
            ...
            float tSp = ((AnalogValue<float>)communicator.BACnetDevice.FindBacnetObject(boID)).m_PROP_PRESENT_VALUE; //この値、いつ使う？
            boID = new BacnetObjectId(BacnetObjectTypes.OBJECT_ANALOG_VALUE, (uint)(10000 + grpNum * 100 + 24));
            float tSpCool = ((AnalogValue<float>)communicator.BACnetDevice.FindBacnetObject(boID)).m_PROP_PRESENT_VALUE;
            boID = new BacnetObjectId(BacnetObjectTypes.OBJECT_ANALOG_VALUE, (uint)(10000 + grpNum * 100 + 25));
            float tSpHeat = ((AnalogValue<float>)communicator.BACnetDevice.FindBacnetObject(boID)).m_PROP_PRESENT_VALUE;
      ...
      lock (communicator.BACnetDevice)
      {
        ...
        for (int i = 0; i < vrfSystems.Length; i++)
        {
          ...
          for (int j = 0; j < vrf.VRFSystem.IndoorUnitNumber; j++)
          {
            ...
            //室内温度設定***************
            boID = new BacnetObjectId(BacnetObjectTypes.OBJECT_ANALOG_VALUE, (uint)(10000 + grpNum * 100 + 24));
            ((AnalogValue<float>)communicator.BACnetDevice.FindBacnetObject(boID)).m_PROP_PRESENT_VALUE = (float)vrf.GetSetpoint(j, true);
            boID = new BacnetObjectId(BacnetObjectTypes.OBJECT_ANALOG_VALUE, (uint)(10000 + grpNum * 100 + 25));
            ((AnalogValue<float>)communicator.BACnetDevice.FindBacnetObject(boID)).m_PROP_PRESENT_VALUE = (float)vrf.GetSetpoint(j, false);
